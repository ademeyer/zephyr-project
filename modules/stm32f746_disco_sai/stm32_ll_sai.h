 ...
 * @{
 */
// #define CLEAR_BIT(REG, BIT_MASK) ((REG) &= ~(BIT_MASK)) /* Clear register bit */
// #define SET_BIT(REG, BIT)        ((REG) |= (BIT))       /* Set register bit */
...
int LL_sai_enable_tx_mute_mode_ll_subsystem(SAI_TypeDef *sai, uint16_t val);
